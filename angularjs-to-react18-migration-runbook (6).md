...
| Sub-components missing after 3-B | 3-D for each missing sub-component | Create new files, update parent imports |
| 3-C flags shared state issue | 3-C targeted fix | Edit specific lines in component |
| 3-C flags missing template coverage | 3-A-4 for the affected region only | Replace that JSX region |
...
Re-run 3-A-5 after all errors are resolved before proceeding to 3-C.

---

### 3-C — Self-Review & Fix
> **Open:** `src/[path]/[ComponentName].jsx` — the fully assembled component after 3-A-5 fixes  
...
> **Before running:** paste parent proposal Section 1 (file path conventions) inline  
> **Note:** Sub-components are small — generate in a single prompt, no need to split
...
Generate a complete React 18 sub-component based on this mini-spec.
Rules:
1. Functional component only — no class components
2. Accept every prop listed in the mini-spec Props section
3. Add PropTypes for every prop
4. Implement every UI Behaviour item from the mini-spec exactly
5. Place the file in the same folder as the parent component
6. Add this comment block at the top:
   // Sub-component of: [ParentComponentName]
   // Extracted from: [original AngularJS template file]
   // Migration date: [today's date]

After the code, list every prop assumption made that was not in the mini-spec.

Output code only. No preamble.
```

**Save output → `src/[path]/[SubComponentName].jsx`**
...
Apply any fixes using the 3-A-5F decision tree, then the parent component is complete.
...
For each missing sub-component (parent before child):
  ...
  3-D-3  →  Generate sub-component from mini-spec
             Save as src/[path]/[SubComponentName].jsx
...
> **Trivially small sub-components** (e.g. a single table row with no logic):  
> Skip 3-D-2 and write the component by hand — it is faster than running the extract prompt.
...
- [ ] 3-C    — Self-review run on `src/[path]/[ComponentName].jsx`, all issues fixed
...
- [ ] 3-D    — ⚠️ Only if proposal Section 3 lists sub-components not yet in `src/`:
  ...
  - [ ] 3-D-3 — Sub-component generated for each
...
*Runbook version 1.9 — AngularJS to React 18 — GitHub Copilot / IntelliJ*  
