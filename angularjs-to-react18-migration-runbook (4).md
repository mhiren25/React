...
Phase 3   — Code Generation        8 prompts      — run per component
            3-A-1: Imports & shell
            3-A-2: State & hooks
            3-A-3: API calls & handlers
...
> **Order:** 3-A-1 → 3-A-2 → 3-A-3 → 3-A-4 → 3-A-5 → 3-C → 3-B  
> **Why 3-A is split:** Generating a complete React component in one prompt causes
> timeouts — especially for layout-level controllers like AppController. Splitting
> into four focused generation prompts keeps each response small and resumable.
...
### 3-A-3 — API Calls & Handlers
> **Open:** `specs/[ComponentName].spec.md`  
> **Before running:** paste spec Section 4, proposal Section 4, and API-Layer-Audit Section 7 inline

```
You are a React 18 engineer. Read #file only.
Do not read any other files. Do not scan the workspace.

Here is the API calls and business rules from the spec (Section 4):
[PASTE spec.md SECTION 4 HERE]
...
Generate ONLY the following inside the component function,
continuing after the hooks already written above:
1. All useEffect hooks for data fetching
2. All event handler functions (handle*, on*)
3. All business logic functions
4. Loading and error state management logic

Preserve every business rule from the spec exactly — do not simplify or omit.
...
- [ ] 3-A-3 — API calls & handlers appended
...
*Runbook version 1.7 — AngularJS to React 18 — GitHub Copilot / IntelliJ*  
